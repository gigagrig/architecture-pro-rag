...
В общий индекс включён [вредоносный тестовый документ](fixtures/prompt_injection.txt). Реализованы защитный системный промпт, удаление подозрительных чанков после поиска, очистка системных конструкций и проверка ответа модели.

Без защиты реальная модель выдала контрольный пароль `swordfish`; с защитой оба провоцирующих вопроса завершились отказом. Итоговая серия: 5 ответов с источниками и 5 корректных отказов, 10 из 10 проверок прошли. API и Telegram-бот обновлены и запущены в Docker.

- [Описание эксперимента, запуск и ограничения защиты](task5_demonstration.md).
- [Скрипт проверки трёх уровней защиты и десяти вопросов](demonstrate_task5.py).
- [Текстовые логи ответов](tasks_artifacts/task5_run.log) и [полные JSON-результаты](tasks_artifacts/task5_results.json).

## Задание 6. Автоматическое ежедневное обновление базы знаний

Реализован [скрипт обновления](update_index.py): SHA-256 обнаруживает изменения в `knowledge_base/entity_*.md`, новые тексты получают эмбеддинги E5, удалённые чанки исключаются. Неизменённые эмбеддинги переиспользуются. Новая версия FAISS публикуется атомарно и подхватывается API перед поиском.

Docker-сервис `index-updater` запускается при старте и ежедневно в 06:00 по Москве; при ошибке повторяет попытку через пять минут. Логи содержат время, количество новых чанков, размер индекса и ошибки.

- [Описание, запуск и ограничения](task6_index_updates.md).
- [Архитектурная диаграмма PlantUML](task6_architecture.puml).
- [Тесты](tests/test_update_index.py) и [лог проверки с реальными эмбеддингами](tasks_artifacts/task6_update.log).

Проверены добавление нового документа и поиск, изменение, удаление, отсутствие изменений и подхват версии без перезапуска Retriever.

## Задание 7. Аналитика покрытия и качества базы знаний

Подготовлен [набор из 11 вопросов](golden_questions.txt): семь известных тем и четыре отказа для базы с пробелами. В экспериментальной копии удалены Xarn Velgor и Void Core вместе с упоминаниями в других документах. [Скрипт оценки](evaluate.py) проверяет исходный, неполный и восстановленный индексы через настоящий RAG-пайплайн.

Постоянное журналирование подключено к API (`runtime/queries.jsonl`). [Журнал эксперимента](logs.jsonl) содержит 44 ответа, найденные и использованные источники, длину, статус и оценку. После восстановления двух сущностей и уточнения статьи Quorin Jaal итоговая оценка исправленной версии — 11/11: девять ответов и два отказа. Ограничения и исправления самого оценщика сохранены в истории результатов.

- [Анализ покрытия, результаты и рекомендации](task7_coverage.md).
- [Итоговая оценка сохранённых ответов](tasks_artifacts/task7_final_assessment.json).
- [Диаграмма последовательности](task7_sequence.puml) и [изображение](task7_sequence.png).
- [Автоматические тесты журналирования и оценки](tests/test_evaluation.py).
